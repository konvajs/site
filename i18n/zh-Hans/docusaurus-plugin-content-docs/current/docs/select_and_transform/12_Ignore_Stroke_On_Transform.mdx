---
title: 如何调整 Canvas 图形尺寸而不改变描边宽度？
sidebar_label: 忽略描边
hide_table_of_contents: true
slug: Ignore_Stroke_On_Transform.html
description: "使用 Konva Transformer 的 ignoreStroke 和 strokeScaleEnabled，防止描边宽度随图形尺寸调整而缩放。"
---

请记住，`Konva.Transformer` 会更改节点的 `scaleX` 和 `scaleY` 属性。
默认情况下，变换图形时，描边也会缩放。在某些情况下，此行为不合适。

有两种方法可以防止描边缩放：

1. 重置图形缩放
2. 结合使用 `shape.strokeScaleEnabled(false)` 和 `transformer.ignoreStroke(false)`

**操作说明：有两个矩形可供调整尺寸。绿色矩形会重置其缩放。红色矩形只会禁用描边缩放。**
