...
expect(stage.find('.selected')).toHaveLength(1);   // .name，不是 CSS 类名
```
...
React 会以异步方式提交来自 Konva 事件处理函数的状态更新，因此请在两次模拟事件之间
让微任务队列清空：
...
afterEach(async () => {
  cleanup();
  // Stage.destroy() 会延迟一个时钟周期，以便 StrictMode 重新挂载时复用它。
...
`{ steps: 10 }` 很重要：一次性跳到目标点可能落在 `dragDistance` 之内，从而根本不会开始拖动。
