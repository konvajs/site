...
此实现结合了[拖动限制示例](/docs/drag_and_drop/Simple_Drag_Bounds.html)和[调整大小限制示例](/docs/select_and_transform/Resize_Limits.html)中的技术，以限制用户交互。
...
const LimitedDragAndResize = () => {
  ...
  const handleTransformerDrag = (e) => {
    ...
    nodes.forEach(shape => {
      ...
      // Repeated drags can clamp to the same state coordinates.
      // Update the node directly because React-Konva skips unchanged props.
      shape.setAbsolutePosition(newAbsPos);
    });
