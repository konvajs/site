...
description: "Improve Konva canvas performance by disabling perfectDrawEnabled to skip buffer canvas rendering for shapes with opacity."
---
...
Using a buffer canvas is extra work on every draw. Since Konva 10.7.0, built-in shapes use a buffer only as large as the shape, and Konva frees it when it is no longer used. A [custom shape](/docs/shapes/Custom.html) gets a buffer the size of the whole canvas, unless it describes its painted area with `selfRectFunc`.

If you draw many translucent shapes with fill and stroke, you can disable such fixing:
