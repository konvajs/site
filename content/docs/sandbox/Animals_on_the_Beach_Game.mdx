...
sidebar_label: Animals on the Beach Game
hide_table_of_contents: true
slug: Animals_on_the_Beach_Game.html
description: "Interactive drag-and-drop game where you match animal images to their silhouettes on a beach using Konva."
---
    ...
    <Image
      ...
      onDragEnd={(e) => {
        ...
        if (!inRightPlace && isNearOutline(newPos, outline)) {
          // The snap target can match a previous state coordinate.
          // Set the node directly because React-Konva skips unchanged props.
          e.target.position({ x: outline.x, y: outline.y });
