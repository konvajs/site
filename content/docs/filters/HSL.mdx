...
const controls = computed(() => [
  { label: 'Hue', prop: 'hue', min: -180, max: 180, value: hslValues.value.hue },
  { label: 'Saturation', prop: 'saturation', min: -2, max: 10, value: hslValues.value.saturation },
  { label: 'Luminance', prop: 'luminance', min: -2, max: 2, value: hslValues.value.luminance },
]);

const updateValue = (prop, value) => {
  hslValues.value[prop] = value;
};
