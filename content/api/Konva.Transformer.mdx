...
description: "Transformer constructor. Transformer is a special type of group that allow you transform Konva primitives and shapes. Transforming tool is not changing width and height properties of nodes when you resize them. Instead it changes scaleX and scaleY properties."
...
| rotateAnchorAngle (optional) | `Number` | Angle of the rotate anchor around the box, in degrees. Default is 0 (top) |
...
| anchorCornerRadius (optional) | `Number` | Anchor corner radius |
...
| boundBoxFunc (optional) | `function` | Bounding box function, see [Konva.Transformer#boundBoxFunc](/api/Konva.Transformer.html#boundBoxFunc) |
| anchorDragBoundFunc (optional) | `function` | Function bounding the position of a dragged anchor, see [Konva.Transformer#anchorDragBoundFunc](/api/Konva.Transformer.html#anchorDragBoundFunc) |
| anchorStyleFunc (optional) | `function` | Function styling every anchor, see [Konva.Transformer#anchorStyleFunc](/api/Konva.Transformer.html#anchorStyleFunc) |
| ignoreStroke (optional) | `Boolean` | Should we ignore stroke size? Default is false |
...
get/set the angle (in degrees) of the rotation anchor position around the bounding box. 0 = top-center (default), 90 = middle-right, 180 = bottom-center, -90 = middle-left
...
get/set bounding box function. **IMPORTANT!** boundBoxFunc operates in absolute coordinates.  Both boxes are `\{ x, y, width, height, rotation \}`, with the rotation in radians.
