...
hexo.extend.helper.register('page_nav', function(){
  ...
  if (index > 0){
    result.push('<a href="/docs/' + keys[index - 1].split('/') + '" id="page-footer-prev" title="' + list[keys[index - 1]] + '">Prev</a>');
  }

  if (index < keys.length - 1){
    result.push('<a href="/docs/' + keys[index + 1] + '" id="page-footer-next" title="' + list[keys[index + 1]] + '">Next</a>');
  }
